...
def pressing_some_buttons(line):
    ...
    jolts = [jolt.strip("{}").split(",") for jolt in line[end].strip("{}").split(",")]

    # now we have the parts we want

    current_light = [False for i in range(len(lights))]
    times_pressed = 0
    best = 9999

    for length in range(1, len(buttons) + 1):
        for collection_of_buttons in itertools.combinations(buttons, length): # only looking at combitations bc order DOESN'T MATTER.... duh

            current_light = [False for i in range(len(lights))]
            times_pressed = 0

            for button in collection_of_buttons:
                current_light = press_button(current_light, button)
                times_pressed += 1
                if times_pressed > 20:
                    print("left loop early")
                    break

            if current_light == lights:
...
def main():
    ...
    with open("../puzzle-inputs/indicator-lights.txt") as f:
